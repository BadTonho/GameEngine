...
The editor is built separately from the runtime when `GAMEENGINE_BUILD_EDITOR=ON` (the
development default). It creates a procedural project without models or textures:

```text
gameengine_editor --new-project <directory>
gameengine_editor --project <directory>/gameengine.geproject
gameengine_editor --project <directory>/gameengine.geproject --smoke-test
```
